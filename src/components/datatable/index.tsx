                  ...
                  <div className="col-span-1 w-full flex justify-end">
                    {actions.map((action, actionIndex) => (
                      <button
                        ...
                        className={`text-black dark:text-white text-sm px-2 py-3 rounded-xl ${action.classNames}`}
                      >
                        <div className="flex flex-row space-x-1">
                          <span>{action.label}</span>
                          {action.icon && <div className="bg-gray-300 p-1 text-black rounded-xl">
                            {action.icon}
                          </div>}
                        </div>
                      </button>
        ...
        <div className="flex justify-center items-center mt-4 space-x-2">
          ...
          {Array.from({ length: totalPages }, (_, idx) => (
            <button
              ...
              className={`px-4 py-2 text-white transition-colors duration-200 rounded-[8px] ${currentPage === idx + 1 ? "bg-[#01a2e4]" : "bg-gray-300"
                }`}
            >
              {idx + 1}
            </button>
