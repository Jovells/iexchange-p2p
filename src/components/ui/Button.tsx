...
const Button: React.FC<ButtonProps> = ({
  ...
  const baseClasses =
    "flex justify-center hoc items-center hover:brightness-110 rounded-[8px] font-medium font-[500px] text-[14px] p-2 px-3";
  ...
  return (
    ...
    >
      ...
      {!loading && iconPosition === "any" && icon && (
        <span className="">
          {typeof icon === "string" ? <img src={icon} alt="icon" className={`h-5 w-5 ${iconClassName}`} /> : icon}
        </span>
      )}
