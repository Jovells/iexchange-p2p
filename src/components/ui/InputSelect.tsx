...
const InputSelect: React.FC<InputSelectProps> = ({
    ...
    return (
        <div ref={dropdownRef} className="relative border border-gray-300 dark:border-gray-600 rounded-[8px] cursor-pointer w-full">
            <div className="w-full flex flex-col">
                {(label && selectedValue) && <span className='text-sm text-gray-500'>{label}</span>}
                <div className="flex items-center w-full px-3 py-3" onClick={toggleDropdown}>
            ...
            {isOpen && (
                <div className="absolute w-full bg-white dark:bg-gray-700 p-2 border border-gray-200 dark:border-gray-600 rounded-[8px] shadow-md z-10">
                    {options.map((option) => (
                        <div
                            key={option.value}
                            className="flex justify-between p-2 border-b border-gray-200 dark:border-gray-600 last:border-b-0 hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer"
                            //@ts-ignore
